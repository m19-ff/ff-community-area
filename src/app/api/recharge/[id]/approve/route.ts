...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'reject') {
      ...
      void sendPushToUsers({
        ...
        payload: {
          ...
          body:  `Your recharge of ${req.amountPoints} pts ($${req.amountUsd}) was rejected.${adminNote ? ' Note: ' + adminNote : ''}`,
    ...
    if (membership) {
      // Player is in a team → credit team wallet only
      ...
      // Audit record on personal wallet (balance unchanged — points go to team)
      ...
      if (wallet) {
        await db.insert(transactions).values({
          ...
          balanceAfter:  wallet.balance,   // personal balance unchanged
      ...
      void sendPushToUsers({
        ...
        payload: {
          ...
          body:  `Your recharge of ${req.amountPoints} pts ($${req.amountUsd}) has been added to your team wallet!`,
          data:  { deepLink: '/wallet', rechargeId: String(reqId) },
        },
        notifType: 'general',
        notifData: { rechargeId: reqId, deepLink: '/wallet' },
    ...
    if (!updated) {
      console.error('[recharge approve] wallet update returned no row for userId', req.userId)
      return apiError('Failed to update wallet', 500)
    }
    ...
    console.error('[POST /recharge/[id]/approve]', error)
    return apiError('Failed to process recharge', 500)
  }
}
